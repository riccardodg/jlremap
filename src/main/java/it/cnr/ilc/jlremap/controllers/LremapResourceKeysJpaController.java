...
public class LremapResourceKeysJpaController implements Serializable {
    ...
    public void create(LremapResourceKeys lremapResourceKeys) throws IllegalOrphanException, PreexistingEntityException, Exception {
        ...
        LremapSubsNorm resourceNormidOrphanCheck = lremapResourceKeys.getResourceNormid();
        if (resourceNormidOrphanCheck != null) {
            LremapResourceKeys oldLremapResourceKeysOfResourceNormid = resourceNormidOrphanCheck.getLremapResourceKeys();
            if (oldLremapResourceKeysOfResourceNormid != null) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("The LremapSubsNorm " + resourceNormidOrphanCheck + " already has an item of type LremapResourceKeys whose resourceNormid column cannot be null. Please make another selection for the resourceNormid field.");
            }
        }
        if (illegalOrphanMessages != null) {
        ...
        try {
            ...
            if (resourceNormid != null) {
                resourceNormid.setLremapResourceKeys(lremapResourceKeys);
    ...
    public void edit(LremapResourceKeys lremapResourceKeys) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (resourceNormidNew != null && !resourceNormidNew.equals(resourceNormidOld)) {
                LremapResourceKeys oldLremapResourceKeysOfResourceNormid = resourceNormidNew.getLremapResourceKeys();
                if (oldLremapResourceKeysOfResourceNormid != null) {
                    if (illegalOrphanMessages == null) {
                        illegalOrphanMessages = new ArrayList<String>();
                    }
                    illegalOrphanMessages.add("The LremapSubsNorm " + resourceNormidNew + " already has an item of type LremapResourceKeys whose resourceNormid column cannot be null. Please make another selection for the resourceNormid field.");
                }
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            ...
            if (resourceNormidOld != null && !resourceNormidOld.equals(resourceNormidNew)) {
                resourceNormidOld.setLremapResourceKeys(null);
                resourceNormidOld = em.merge(resourceNormidOld);
            }
            if (resourceNormidNew != null && !resourceNormidNew.equals(resourceNormidOld)) {
                resourceNormidNew.setLremapResourceKeys(lremapResourceKeys);
    ...
    public void destroy(String id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            LremapSubsNorm resourceNormid = lremapResourceKeys.getResourceNormid();
            if (resourceNormid != null) {
                resourceNormid.setLremapResourceKeys(null);
                resourceNormid = em.merge(resourceNormid);
            }
